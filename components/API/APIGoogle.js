...
class APIGoogle {
    ...
    async getAdressesFromLatitudeAndLongitude(latitude, longitude) {
        var request = APIBaseURL + 'geocode/json?address=' + latitude + ',' + longitude + '&key=' + APIkeyGoogle
        console.log(request)
        try{
    ...
    async getAddressFromLocation(latitude, longitude) {
        var address = this.extractFirstAddressFromResponse(await this.getAdressesFromLatitudeAndLongitude(latitude, longitude))
        return address
    }
